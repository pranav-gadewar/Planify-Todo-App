...
export default function DashboardPage() {
  ...
  const groupTasksByDate = (tasks: Task[]) => {
    ...
    tasks.forEach((task) => {
      const date = task.createdAt
        ? task.createdAt.toDate().toLocaleDateString(undefined, {
            weekday: "long",
            year: "numeric",
            month: "long",
            day: "numeric",
  ...
  return (
    <div className="w-full min-w-0 overflow-x-hidden max-w-7xl mx-auto px-4 py-6 sm:px-6 sm:py-8 lg:px-8 space-y-8">

      {/* Header */}
      <motion.div
        initial={{ opacity: 0, x: -20 }}
        animate={{ opacity: 1, x: 0 }}
        className="flex flex-col md:flex-row md:items-end justify-between gap-6 min-w-0"
      >
        <div className="min-w-0">
          ...
          <p className="text-gray-400 mt-2 text-base max-w-md">
            Welcome back! Here&apos;s a summary of your productivity and
            upcoming tasks.
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
          ...
          <Link
            href="/dashboard/addTask"
            className="w-full sm:w-auto flex items-center justify-center gap-2 px-5 py-3 rounded-xl bg-white text-black font-semibold hover:bg-gray-200 transition-all shadow-lg shadow-white/5"
          >
            <Plus size={18} strokeWidth={3} />
            Add Task
          </Link>
          ...
          <Link
            href="/dashboard/tasks"
            className="w-full sm:w-auto flex items-center justify-center px-5 py-3 rounded-xl bg-purple-600/10 border border-purple-500/30 text-purple-300 font-medium hover:bg-purple-600/20 transition-all"
          >
            View All
          </Link>
                ...
                <div className="bg-white/5 border border-white/10 rounded-2xl divide-y divide-white/5 overflow-hidden shadow-xl">
                  ...
                  {groupedTasks[date].map((task) => (
                    <motion.div
                      ...
                      className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 p-5 transition-all group min-w-0"
                    >
                      ...
                      <div className="flex items-center justify-between sm:justify-end gap-4 shrink-0">
                        ...
                        <ArrowRight
                          size={16}
                          className="text-white/0 group-hover:text-white/20 transition-all -translate-x-2 group-hover:translate-x-0"
                        />
